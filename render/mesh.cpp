...
bool Mesh::loadFromOBJ(const char *path){
    LOG_DEBUG("Loading OBJ file %s...\n", path);

    std::vector<unsigned int> vertexIndices, uvIndices, normalIndices;
    std::vector<math::Vector3> temp_vertices;
    std::vector<math::Vector3> temp_uvs;
    ...
    while( 1 ){
        ...
        if ( strcmp( lineHeader, "v" ) == 0 ){
            ...
            if (matches != 9){
                LOG_DEBUG("File can't be read by our simple parser :-( Try exporting with other options\n");
                return false;
            }
    ...
    for( unsigned int i=0; i<vertexIndices.size(); i++ ){
        ...
        math::Vector3 vertex = temp_vertices[ vertexIndex-1 ];
        math::Vector2 uv = temp_uvs[ uvIndex-1 ];
        math::Vector3 normal = temp_normals[ normalIndex-1 ];
